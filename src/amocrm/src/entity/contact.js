...
class Contact extends BaseEntity {
    ...
    async update(params = {
        ...
        return response.data._embedded ? response.data._embedded.contacts : response.data;
    }

    async chatFind(params = {
        access: {
            domain: undefined,
            token: undefined
        },
        ...
        if (response.status !== 204)
            return response.data._embedded.chats;
    }

    async chatLink(params = {
        access: {
            domain: undefined,
            token: undefined
        },
        ...
        if (!params.rawData && (!params.link || !params.link.contact_id || !params.link.chat_id))
            throw new Error("Not filled in request body data");
